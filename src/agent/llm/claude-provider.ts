...
// Small: a tip decision plus an adaptive-thinking summary fits comfortably; keeps
// the request non-streaming and well under the SDK HTTP-timeout window.
  ...
  async reason(observation: AgentObservation): Promise<LlmResult> {
    ...
    for (let attempt = 0; attempt < ClaudeProvider.MAX_RETRIES; attempt++) {
      try {
        ...
        return extractClaudeReasoning(response.content as unknown as ClaudeBlock[]);
      } catch (err) {
